...
ps <- lapply(seq_along(ds), function(i) {
  p <- ggplot(ds[[i]]) + 
    ...
    #theme(legend.title=element_text(ds[[i]]$var[1]), legend.position=c(0.1, 0.9))
...
getcc(readRDS("results_glucose.RDS")$Breed$model)
getcc(readRDS("results_insulin.RDS")$Breed$model)
```

Now need to compute contrasts of interest, test using chisquared test, and correct for multiple comparisons